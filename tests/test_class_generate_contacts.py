...
class TestMergeData:
    ...
    def test_blank_srnum(self):
        """
                check if any blank serial number is available in output
                """
        result = obj_contact.generate_contacts()
        null_valeus = result['Serial_Number'].isnull().any()
        expected_output = False
        assert null_valeus == expected_output

    def test_blank_source(self):
        """
                check if any blank source is available in output
                """
        result = obj_contact.generate_contacts()
        null_valeus = result['Source'].isnull().any()
        expected_output = False
        assert null_valeus == expected_output
