...
class ProcessServiceIncidents:
    ...
    def main_services(self):  # pragma: no cover
        ...
        try:
            ...
            loggerObj.app_success(_step)
            loggerObj.app_info(f"Output of validate_srnum in main_services is {validate_srnum['SerialNumber'].unique()}")
            ...
            jcomm_sidecar_obj = self.pipeline_component_identify(df_services_raw)
            loggerObj.app_info("Now calling pipline_component_identify function defined in class_services_data.py")
    ...
    def pipeline_id_hardwarechanges(self, df_data, dict_filt,
        ...
        try:
            ...
            # ls_cols_interest = ["Id","Status","ClosedDate","Customer_Issue_Summary__c","Customer_Issue__c","Resolution_Summary__c","Resolution__c","SerialNumberOrg","SerialNumberContract","Qty","src","SerialNumber","KeySerial"]
            df_out = pd.DataFrame()
            ...
            df_data["list_of_strings_for_summary"] = df_data["Customer_Issue_Summary__c"].apply(
                                         lambda x : re.split('\n', str(x)))
            loggerObj.app_info("Inside function pipeline_id_hardwarechanges defined in class_servivces_data.py and beginning with for loop")
            ...
            for component in dict_filt:
                ...
                if any(df_data.f_all):
                    loggerObj.app_info("Inside function pipeline_id_hardwarechanges defined in class_servivces_data.py, Inside if block")
                    ...
                    del df_data_comp
                    loggerObj.app_info("Inside function pipeline_id_hardwarechanges defined in class_servivces_data.py,coming out of the if block.")
                ...
                loggerObj.app_info(f"Completed iteration for the component {component} pipeline_id_hardwarechanges defined in class_servivces_data.py")
        except Exception as excep:
