...
class BusinessLogic:
    ...
    def idetify_product_fr_serial(self, ar_serialnumber):
        ...
        for prod in ls_prod:
            # prod = ls_prod [0]
            ls_pattern = ref_prod.loc[ref_prod.Product == prod, "SerialNumberPattern"]
    ...
    def idetify_product_fr_tln(self, ar_tln):
        ...
        ref_prod = ref_prod[(ref_prod.flag_keep) & (pd.notna(ref_prod.PartNumber_TLN))]
        ...
        for prod in ls_prod:
            # prod = ls_prod [0]
            ls_pattern = ref_prod.loc[(ref_prod.Product == prod), "PartNumber_TLN"]

            df_data["flag_prod_beg"] = df_data.key.str.startswith(tuple(ls_pattern))
            df_data["flag_prod_con"] = df_data.key.apply(
                lambda x: re.search(f"({str.lower(prod)})", x) is not None
            )
            df_data["flag_prod"] = df_data["flag_prod_beg"] | df_data["flag_prod_con"]
